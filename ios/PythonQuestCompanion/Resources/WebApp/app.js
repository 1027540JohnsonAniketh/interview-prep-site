...
const RUNTIME_CLIENT = window.PythonQuestRuntimeClient.createClient();
const IS_OFFLINE_RUNTIME = Boolean(RUNTIME_CLIENT.isOffline);
...
createApp({
    ...
    heroContent() {
      ...
      if (this.activeWorkspace === "python") {
        return {
          ...
          title: IS_OFFLINE_RUNTIME
            ? "Run the Python Course Entirely On Device"
            : "Run the Existing Lesson CLI Inside the Browser",
          copy:
            IS_OFFLINE_RUNTIME
              ? "The iOS app now ships with the lesson data, Python challenge validators, and a local terminal engine, so Quest and Lab work without Render or your Mac."
          ...
          metrics: [
            ...
            {
              label: "CLI Access",
              value: this.pythonAvailable ? (IS_OFFLINE_RUNTIME ? "On Device" : "Live") : "Local",
            },
  ...
  methods: {
    ...
    async fetchPythonQuestCatalog(force = false) {
      ...
      try {
        const payload = await RUNTIME_CLIENT.fetchQuestCatalog();
    ...
    async submitQuestCode(lesson) {
      ...
      try {
        const payload = await RUNTIME_CLIENT.validateQuest(lesson.slug, challenge.index, code);
    ...
    async fetchPythonStatus(force = false) {
      ...
      try {
        const payload = await RUNTIME_CLIENT.getPythonStatus();
    ...
    async startPythonSession() {
      ...
      try {
        const payload = await RUNTIME_CLIENT.createPythonSession();
    ...
    async pollPythonOutput() {
      ...
      try {
        const payload = await RUNTIME_CLIENT.readPythonOutput(this.pythonSessionId, this.pythonCursor);
    ...
    async sendPythonInput(overrideText = null) {
      ...
      try {
        await RUNTIME_CLIENT.sendPythonInput(this.pythonSessionId, text);
        if (RUNTIME_CLIENT.isOffline) {
          await this.pollPythonOutput();
        }
      } catch (error) {
        if (String(error?.message || "").toLowerCase().includes("not found")) {
          this.markPythonSessionClosed("Python CLI session not found. Start a new session.");
    ...
    async stopPythonSession(quiet = false) {
      ...
      try {
        await RUNTIME_CLIENT.deletePythonSession(sessionId);
      } catch (error) {
    ...
    async fetchPayload() {
      try {
        this.loading = true;
        this.error = "";
        this.payload = await RUNTIME_CLIENT.fetchSections();
      } catch (error) {
    ...
    <div class="page-shell">
      ...
      <main class="content" v-if="!loading && !error">
        ...
        <section class="python-lab" v-else>
          ...
          <p>${IS_OFFLINE_RUNTIME
            ? "This app is using the bundled on-device lesson engine, so Quest and Lab work without any remote server."
            : "This runs your existing python/run.py lesson CLI inside the app. Pick a lesson and mode exactly like terminal usage."}</p>
